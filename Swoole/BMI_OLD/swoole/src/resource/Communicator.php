...
class Communicator
{
    ...
    public function initialize()
    {
		...
		$timeOut = !empty($ice['timeOut']) ? $ice['timeOut'] : 2000;
        $iceGrid = !empty($ice['iceGrid']) ? $ice['iceGrid'] : '';
        $initData = new \Ice_InitializationData;
        $initData->properties = Ice_createProperties();
        $initData->properties->setProperty('Ice.Default.Timeout', $timeOut);
        $initData->properties->setProperty('Ice.Override.Timeout', $timeOut);
        $initData->properties->setProperty('Ice.Default.InvocationTimeout', $timeOut);
        if($iceGrid){
